...
func _load_game_info(game_dir_path) -> GameData:
	...
	while true:
		...
		if !has_executable and current_file.begins_with("Ejecutable") and _is_executable(full_path):
			has_executable = true
	...
	if !has_executable or !has_info or !has_cover:
		...
		push_error("couldn't load game for folder " + game_dir_path + ", missing " + suffix + ".")
		return null
